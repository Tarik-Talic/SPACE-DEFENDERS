var KEY_CODE_LEFT= 37;
var KEY_CODE_RIGHT= 39;
var KEY_CODE_SPACE= 32;

var GAME_WIDTH =800;
var GAME_HEIGHT=600;

var PLAYER_WIDTH = 20;
var PLAYER_MAX_SPEED=600.0;
var LASER_MAX_SPEED=300.0;
var LASER_COOLDOWN= 0.5;

var ENEMIES_PER_ROW =10;
var ENEMY_HORIZONTAL_PADDING = 80;
var ENEMY_VERTICAL_PADDING =70;
var ENEMY_VERTICAL_SPACING = 80;
var ENEMY_COOLDOWN = 7.0;
  

var GAME_STATE={
playerX:0,
playerY:0,
playerCooldown: 0,
lasers:[],
enemyLasers:[],
enemies:[],
leftPressed: false,
rightPressed: false,
spacePressed: false,
lastTime: Date.now(),
gameOver: false
};

function rectsIntersect(r1, r2){
    return !(
        r2.left > r1.right ||
        r2.right < r1.left ||
        r2.top > r1.bottom ||
        r2.bottom < r1.top
    );
}

function setPosition($el,x,y) {
    $el.style.transform =`translate(${x}px, ${y}px)`;
}

function clamp(v,min,max) {
    if (v < min){
        return min;
    }else if(v > max){
        return max;
    }else {
        return v;
    }
}

function rand(min,max){
    if(min === undefined) min=0;
    if(max === undefined) max = 1;
    return min + Math.random() * (max-min);
}
 




function createPlayer($container){
GAME_STATE.playerX = GAME_WIDTH / 2;
GAME_STATE.playerY = GAME_HEIGHT - 50;
var player =document.createElement("img");
player.src= "img/player-red-1.png";
player.className = "player";
$container.appendChild(player);
setPosition(player , GAME_STATE.playerX, GAME_STATE.playerY);
}

function destroyPlayer($container,player){
    $container.removeChild(player);
    GAME_STATE.gameOver = true;
    var audio = new Audio("sound/sfx-lose.ogg");
    audio.play();
}
function wonPlayer($container,player){
    $container.removeChild(player);
    GAME_STATE.gameWon = true;
    var audio = new Audio("sound/sfx-lose.ogg");
    audio.play();
}



function updatePlayer(dt, $container){
    if(GAME_STATE.leftPressed) {
        GAME_STATE.playerX -= dt * PLAYER_MAX_SPEED;
    }
     if(GAME_STATE.rightPressed){
        GAME_STATE.playerX += dt * PLAYER_MAX_SPEED;
    }

    GAME_STATE.playerX = clamp(GAME_STATE.playerX, PLAYER_WIDTH, GAME_WIDTH-PLAYER_WIDTH);

    if(GAME_STATE.spacePressed && GAME_STATE.playerCooldown <= 0){
        createLaser($container,GAME_STATE.playerX,GAME_STATE.playerY);
        GAME_STATE.playerCooldown = LASER_COOLDOWN;
    }
    if (GAME_STATE.playerCooldown > 0){
        GAME_STATE.playerCooldown -= dt;
    }

    var $player=document.querySelector(".player");
    setPosition($player , GAME_STATE.playerX, GAME_STATE.playerY);
}

function createLaser($container, x , y){
var $element = document.createElement("img");
$element.src = "img/laser-blue-16.png";
$element.className = "laser";
$container.appendChild($element);
var laser ={ x, y, $element };
GAME_STATE.lasers.push(laser);
var audio = new Audio("sound/sfx-laser1.ogg");
audio.play();
setPosition($element,x,y);
}

function updateLasers(dt, $container){ 
    var lasers = GAME_STATE.lasers;
    for (var i = 0; i < lasers.length; i++){
        var laser = lasers[i];
        laser.y -= dt * LASER_MAX_SPEED;
        if (laser.y <0){
            deleteLaser($container, laser);
        }
        setPosition(laser.$element, laser.x , laser.y);
        var r1 = laser.$element.getBoundingClientRect();
        var enemies = GAME_STATE.enemies;
        for (var j=0; j < enemies.length; j++){
            var enemy = enemies[j];
            if (enemy.isDeleted) continue;
            var r2 = enemy.$element.getBoundingClientRect();
            if(rectsIntersect(r1,r2)){
                destroyEnemy($container, enemy);
                deleteLaser($container, laser);
                break;
            }
        }
    }
    GAME_STATE.lasers = GAME_STATE.lasers.filter(e => !e.isDeleted);
}

function deleteLaser($container,laser){
    $container.removeChild(laser.$element);
    laser.isDeleted = true;
}

function createEnemy($container,x,y){
var $element =document.createElement("img");
$element.src="img/ufoGreen.png";
$element.className="enemy";
$container.appendChild($element);
var enemy ={
    x,
    y,
    cooldown: rand(0.5, ENEMY_COOLDOWN),
    $element
};
GAME_STATE.enemies.push(enemy);
setPosition($element,x,y);
}

function updateEnemies(dt,$container){
    var dx = Math.sin(GAME_STATE.lastTime / 1000.0)* 50;
    var dy = Math.cos(GAME_STATE.lastTime / 1000.0)* 10;

    var enemies = GAME_STATE.enemies;
    for(var i = 0; i<enemies.length;i++){
        var enemy = enemies[i];
        var  x = enemy.x + dx;
        var  y = enemy.y + dy;
        setPosition(enemy.$element, x, y);
        enemy.cooldown -= dt;
        if (enemy.cooldown <= 0){
            createEnemyLaser($container, x , y);
            enemy.cooldown = ENEMY_COOLDOWN;
        }
    }
    GAME_STATE.enemies = GAME_STATE.enemies.filter(e => !e.isDeleted);
}

function destroyEnemy($container,enemy){
    $container.removeChild(enemy.$element);
    enemy.isDeleted = true;
    var audio = new Audio("sound/sfx_twoTone.ogg");
    audio.play();
}

function createEnemyLaser($container, x ,y){
    var $element = document.createElement("img");
    $element.src="img/laser-red-7.png";
    $element.className="enemy-laser";
    $container.appendChild($element);
    var laser = {x, y, $element};
    GAME_STATE.enemyLasers.push(laser);
    setPosition($element,x,y);
}

function updateEnemyLasers(dt,$container){
    var lasers = GAME_STATE.enemyLasers;
    for ( var i = 0; i< lasers.length; i++){
        var laser = lasers[i];
        laser.y += dt * LASER_MAX_SPEED;
        if (laser.y > GAME_HEIGHT){
            deleteLaser($container, laser);
        }
        setPosition(laser.$element, laser.x, laser.y);
        var r1 = laser.$element.getBoundingClientRect();
        var player = document.querySelector(".player");
        var r2 = player.getBoundingClientRect();
        if (rectsIntersect(r1,r2)){
            destroyPlayer($container, player);
            break;
        }
    }
    GAME_STATE.enemyLasers = GAME_STATE.enemyLasers.filter(e => !e.isDeleted );
}


function init(){
var $container=document.querySelector(".game");
createPlayer($container);


var enemySpacing = (GAME_WIDTH - ENEMY_HORIZONTAL_PADDING * 2) /
(ENEMIES_PER_ROW - 1);
for(var j=0;j<3;j++){
    var y = ENEMY_VERTICAL_PADDING + j * ENEMY_VERTICAL_SPACING;
    for(var i =0;i<ENEMIES_PER_ROW;i++){
        var x = i * enemySpacing + ENEMY_HORIZONTAL_PADDING;
        createEnemy($container,x,y);
    }
  }
}

function playerHasWon() {
    return GAME_STATE.enemies.length === 0;

  }

function update(e){
    var currentTime= Date.now();
    var dt =(currentTime - GAME_STATE.lastTime) / 1000;
        
    if(GAME_STATE.gameOver){
        document.querySelector(".game-over").style.display="block";
        var audio = new Audio("sound/Lose-music.mp3");
        audio.play();
        return;
    }

    if (playerHasWon()) {
        document.querySelector(".congratulations").style.display = "block";
        var audio = new Audio("sound/Win-music.mp3");
        audio.play();
        return;
      }
    var $container = document.querySelector(".game");
    updatePlayer(dt, $container);
    updateLasers(dt , $container);
    updateEnemies(dt ,$container);
    updateEnemyLasers(dt , $container); 

    GAME_STATE.lastTime =currentTime;
    window.requestAnimationFrame(update);
}


function onKeyDown(e) {
   if (e.keyCode === KEY_CODE_LEFT){
       GAME_STATE.leftPressed = true;
   }else if(e.keyCode === KEY_CODE_RIGHT){
       GAME_STATE.rightPressed = true;
   }else if(e.keyCode === KEY_CODE_SPACE){
       GAME_STATE.spacePressed = true;
   }
       
}

function onKeyUp(e) {
    if (e.keyCode === KEY_CODE_LEFT){
        GAME_STATE.leftPressed = false;
    }else if(e.keyCode === KEY_CODE_RIGHT){
        GAME_STATE.rightPressed = false;
    }else if(e.keyCode === KEY_CODE_SPACE){
        GAME_STATE.spacePressed = false;
    }
}

init();
window.addEventListener("keydown",onKeyDown);
window.addEventListener("keyup",onKeyUp);
window.requestAnimationFrame(update);
